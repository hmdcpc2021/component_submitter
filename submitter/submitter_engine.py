...
class SubmitterEngine(object):
    ...
    def query(self, query, app_id, dry_run=False):
        """ Query method to query the adaptors for a specific application """
        result_dict = {}
        adaptors = self._instantiate_adaptors(app_id, dry_run)

        for adaptor_name, adaptor in adaptors.items():
            try:
                result = adaptor.query(query)
                result_dict[adaptor_name] = result
            except AttributeError:
                continue

        if result_dict:
            self.app_list.setdefault(app_id, {}).setdefault("output", {}).update(result_dict)
            self._update_json()  # Update the JSON file with the new output data
            return result_dict
        else:
